...
/*
  ...
  - m/d/[yy[yy]]: examples: '7/21', '7/21/07', '7/21/2007'
...
function string_to_date(s) {
  ...
  if (mo = s.match(/^\s*(\d{4})-(\d{1,2})-(\d{1,2})\s*$/)) {
    ...
    // 'm/d/yyyy' format and abbreviations.
    mo[3] = mo[3] || today.getFullYear();
    mo[3] = Number(mo[3]);
    if (mo[3] < 50) mo[3] += 2000;
    result = new Date(mo[3], Number(mo[1])-1, mo[2]);
  }
  return result;
}
...
function calendar_show() {
  calendar.element.style.zIndex = 2001;
