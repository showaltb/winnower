...
class Date
  ...
  #
  # * US date format: mm/dd or mm/dd/yy or mm/dd/yyyy
  ...
  def self.parse_date(value)
    ...
    today = Date.today
    if string =~ /^(\d{1,2})\/(\d{1,2})(?:\/(\d{2}(?:\d{2})?))?$/
      month, day, year = $1.to_i, $2.to_i, ($3 || today.year).to_i
      # if year is 2 digits, set to current century and adjust to fall
      # within window
      if year < 100
        year += Date.today.year / 100 * 100
        year -= 100 while year > Date::WINDOW_END
        year += 100 while year < Date::WINDOW_BEG
      end
      begin
  ...
  def validates_dates(*attr_names)
    configuration =
      { :message => 'is an invalid date ' \
                    '(here are some valid examples: 2/23/06, 2/23, 23, 23 feb, 23 feb 06, ' \
    ...
    validates_each(attr_names, configuration) do |record, attr_name, value|
      ...
      next if allow_nil and (before_cast.blank?)
      begin
